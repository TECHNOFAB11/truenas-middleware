...
class RoleManager:
    ...
    def roles_for_fole(self, role):
        if role not in self.roles:
            return set()

        return set.union({role}, *[self.roles_for_fole(included_role) for included_role in self.roles[role].includes])

    def allowlist_for_role(self, role):
        return sum([
            self.allowlists_for_roles[role]
            for role in self.roles_for_fole(role)
        ], [])
